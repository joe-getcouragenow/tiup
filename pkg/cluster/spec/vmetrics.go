...
// VictoriaMetricsSpec represents the VictoriaMetrics Server topology specification in topology.yaml
...
func (s VictoriaMetricsSpec) Role() string {
	return ComponentVictoriaMetrics
}

// SSH returns the host and SSH port of the instance
func (s VictoriaMetricsSpec) SSH() (string, int) {
	return s.Host, s.SSHPort
}

// GetMainPort returns the main port of the instance
func (s VictoriaMetricsSpec) GetMainPort() int {
	return s.Port
}

// IsImported returns if the node is imported from TiDB-Ansible
func (s VictoriaMetricsSpec) IsImported() bool {
	return s.Imported
}

// VictoriaMonitorComponent represents Monitor component.
type VictoriaMonitorComponent struct{ Topology }

// Name implements Component interface.
func (c *VictoriaMonitorComponent) Name() string {
	return ComponentVictoriaMetrics
}

// Role implements Component interface.
func (c *VictoriaMonitorComponent) Role() string {
	return RoleMonitor
}

// Instances implements Component interface.
func (c *VictoriaMonitorComponent) Instances() []Instance {
	servers := c.BaseTopo().Monitors
	ins := make([]Instance, 0, len(servers))

	for _, s := range servers {
		ins = append(ins, &VictoriaMonitorInstance{BaseInstance{
			InstanceSpec: s,
			Name:         c.Name(),
			Host:         s.Host,
			Port:         s.Port,
...
// VictoriaMonitorInstance represent the monitor instance
type VictoriaMonitorInstance struct {
	BaseInstance
	topo Topology
}

// InitConfig implement Instance interface
func (i *VictoriaMonitorInstance) InitConfig(
	e executor.Executor,
	clusterName,
	clusterVersion,
	deployUser string,
	...
	// transfer run script
	spec := i.InstanceSpec.(VictoriaMetricsSpec)
	cfg := scripts.NewVictoriaMetricsScript(
		i.GetHost(),
		paths.Deploy,
		paths.Data[0],
		paths.Log,
	...
	cfig := config.NewVictoriaMetricsConfig(clusterName, enableTLS)
...
// rules for dm.
func (i *VictoriaMonitorInstance) installRules(e executor.Executor, deployDir, clusterVersion string) error {
...
func (i *VictoriaMonitorInstance) initRules(e executor.Executor, spec VictoriaMetricsSpec, paths meta.DirPaths) error {
	if spec.RuleDir != "" {
...
func (i *VictoriaMonitorInstance) ScaleConfig(
	e executor.Executor,
	topo Topology,
	clusterName string,
